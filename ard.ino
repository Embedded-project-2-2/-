...
void captureFrame(unsigned int frameWidth, unsigned int frameHeight) {
  ...
  while (frameHeight--) {
    ...
    while (tempWidth--) {
      ...
      Serial.write(byteToWrite); // send data via serial connection with UART register
      delayMicroseconds(1); // small delay to avoid buffer overflow
    }
  }
...
void loop() {
  captureFrame(640, 480); // capture a frame at QVGA resolution (320 x 240)
}